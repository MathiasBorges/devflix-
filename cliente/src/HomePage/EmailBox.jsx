        ...
        if (email === "") {
            ...
            try {
                const response = await axios.post(
                    'https://devflix-api-gd6r.onrender.com/prelogin' || "http://192.168.0.138:3000/prelogin",
        ...
        <>
            <div className="mainBox">
                ...
                <form className='form-Home' onSubmit={fazerLogin} action='#'>
                    <input type="email" name="email" placeholder="Email" required onChange={(elemento) => setEmail(elemento.target.value)} />
                    <input type="submit" value="Vamos lá!"  onClick={(elemento)=>elemento.target.style.animation="carregaBotao 2.5s linear alternate"}/>
                </form>
            </div>
        </>
    )
